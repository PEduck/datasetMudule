...
def cloth_color(image_path, expected_size=40, in_clusters=7, out_clusters=3, final_size=300):
    ...
    print(image.shape)
    initial_image = image[:, :, :3]
    A_chenal = image[:, :, 3]

    initial_image = cv2.bitwise_and(initial_image, initial_image, mask=A_chenal)
    
    # initial_image = cv2.colorChange(initial_image, cv2.COLOR_RGBA2BGR)
    print(initial_image.shape)
    # cv2.imshow('src_img', initial_image)
    ...
    if initial_image is not None:
        ...
        # if max(src_w, src_h)/min(src_w, src_h)<1.4:
        #     frame_y = int(src_h*0.02)
        #     frame_x = int(src_w*0.02)
        # else:
        #     frame_y = int(src_h * 0.02)
        #     frame_x = int(src_w * 0.02)
        # frame_y = 0
        ...
        in_samples = []
        # border_samples = []
        # limit_Y = LAB_image.shape[0] - frame_y
        # limit_X = LAB_image.shape[1] - frame_x

        for y in range(LAB_image.shape[0] - 1):
            for x in range(LAB_image.shape[1] - 1):
                pt = LAB_image[y][x]
                # 去除白色像素值
                if pt[0] == 0 and pt[1]==128 and pt[2]==128:
                    # print(pt)
                    continue
                # if x < frame_x or y < frame_y or y >= limit_Y or x >= limit_X:
        ...
        for i in range(in_len):
            ...
            count_in_label_i = count_in_labels.get(i, 0)
            if count_in_label_i == 0:
                valid[i] = False
                continue
        ...
        for i, color_LAB in enumerate(colors):
            ...
            print(proportions[i]/float(total_color))
...
if __name__ == "__main__":
    img_path = '0003df41-503c-492f-afbf-5dd3db0b1b9e-524099_1.png'
    img_path = '/nas/liwenbo/xtc_80w_seg/21/257/0003df41-503c-492f-afbf-5dd3db0b1b9e-524099_1.png'
    # cloth_color(sys.argv[1:][0])
    print(cloth_color(img_path))
